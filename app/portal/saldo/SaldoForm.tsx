...
export default function SaldoForm({ isAdmin, buildings, defaultBuilding, defaultUnit }: Props) {
  const [building, setBuilding] = useState(defaultBuilding.toLowerCase())
  const [unit, setUnit] = useState(defaultUnit.toLowerCase())
  const [result, setResult] = useState<SaldoResult | null>(null)
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(false)

  useEffect(() => {
    setBuilding(defaultBuilding.toLowerCase())
    setUnit(defaultUnit.toLowerCase())
  }, [defaultBuilding, defaultUnit])
  ...
  const formatSaldo = (saldo: number, moneda: 'UYU' | 'USD') => {
    if (moneda === 'UYU') return `$ ${saldo.toLocaleString('es-UY')}`
      ...
      {result && (
        <div
          className={`mt-4 rounded-2xl border p-6 ${
            result.saldo === 0 ? 'bg-green-50 border-green-200' : 'bg-white border-ayala-light'
          }`}
        >
